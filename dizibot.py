...
vt = sqlite3.connect("dizi_veri.sqlite3")
fvt = vt.cursor()
hata=0
page = 1
while page<690:
    ...
    j = 0
    url = requests.get("https://www.beyazperde.com/diziler/tum/?page="+str(page))
    ...
    for movie in movies:
        ...
        try:
            date = dom.xpath('//*[@id="content-layout"]/section[2]/div[2]/ul/li['+str(i[j])+']/div/div[1]/div/div/text()')[0]
            date = date.strip()
            if ":" in date:
                date = date.split(":")[1]
        except:
        ...
        fvt.execute("INSERT INTO dizi (name,creater,kind,actor,img_url,start_time,content) VALUES (?,?,?,?,?,?,?)",(name,vdirector,vkind,vactor,img_name,date,content))
        vt.commit()
        j+=1
